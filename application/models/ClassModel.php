...
class ClassModel extends CI_Model{
    ...
    public function create($data){
        $class = $this->getClassByName($data);
        if(count($class) == 0){
            if($this->db->insert($this->table, $data)){
                $query = $this->getClassByName($data);
                $classEdit = array(
                    "id_class" => $query->id_class,
                    "name_class" => $query->name_class,
                    "number_class" => $this->encryption($query->id_class)
                );
                $this->update($classEdit);
